...
//check if enum type 
    ...
    public function obtenerDatos($lng) {
        $sql = $this->db->select("SELECT nombre_empresa FROM `datos` WHERE id = 1;");
        return $sql[0];
    }

    public function cargarMenu($lng) {
        $sql = $this->db->select("SELECT id, " . $lng . "_texto as texto, controlador, metodo, funcion from menu where estado = 1 ORDER BY orden ASC");
        return $sql;
    }
    ...
    public function cargarSlider($lng) {
        $sql = $this->db->select("SELECT
                                        imagen,
                                        " . $lng . "_data_title as data_title,
                                        " . $lng . "_titulo as titulo,
                                        " . $lng . "_titulo_principal as titulo_principal,
                                        " . $lng . "_descripcion as descripcion,
                                        " . $lng . "_texto_boton as texto_boton,
                                        " . $lng . "_url_boton as url_boton
                                FROM
                                        `slider`
                                WHERE
                                        estado = 1
    ...
    public function cargarProductos($lng) {
        $sql = $this->db->select("SELECT
                                        " . $lng . "_producto as producto,
                                        imagen
                                FROM
                                        `productos`
                                WHERE
    ...
    public function getLogos() {
        $sql = $this->db->select("select * from logo where id = 1");
        return $sql[0];
    }

    public function obtenerDatosResponsabilidad($lng) {
        $sql = $this->db->select("SELECT
                                        " . $lng . "_titulo as titulo,
                                        " . $lng . "_contenido as contenido,
                                        " . $lng . "_titulo_granjero as titulo_granjero,
                                        " . $lng . "_contenido_granjero as contenido_granjero,
                                        imagen_1,
                                        imagen_2,
                                        imagen_3,
                                        imagen_fondo
    ...
    public function obtenerDatosBeneficiosResponsabilidad($lng) {
        $sql = $this->db->select("SELECT imagen, " . $lng . "_titulo as titulo FROM `beneficios_productores` where estado = 1 ORDER BY orden ASC;");
        return $sql;
    }

    public function obtenerEntradasBlog($lng, $limit = 3) {
        $sql = $this->db->select("SELECT
                                        id,
                                        " . $lng . "_titulo as titulo,
                                        SUBSTR(" . $lng . "_contenido FROM 1 FOR 190) AS contenido,
                                        DATE_FORMAT(fecha_blog,'%M %d, %Y') as fecha,
    ...
    public function footerProductos($lng) {
        $sql = $this->db->select("SELECT id, " . $lng . "_producto as producto FROM `productos` where estado = 1 ORDER BY orden ASC;");
        return $sql;
    }

    public function footerServicios($lng) {
        $sql = $this->db->select("SELECT id, " . $lng . "_servicio as servicio FROM `servicios` WHERE estado = 1 ORDER BY orden ASC;");
        return $sql;
    }
